...
@Component({
  ...
  templateUrl: './bids-viewer.component.html',
  styleUrls: ['./bids-viewer.component.scss']
})


export class BidsViewerComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      this.dateFormatted = this.formatDate(this.transactionDate);
      
      console.log('Route Params:', {
        bidHistoryId: this.bidHistoryId,
  ...
  CopyOfferRR(): void {
    ...
    if (this.bidArchiveRows && this.bidArchiveRows.length > 0) {
      this.offers = [...this.bidArchiveRows];
      this.offer = this.offers[0];
  
  
      this.isInvalidFile = false;
      this.successUploadMessage = "BID Schedule Copied.";
  ...
  loadBidArchive(bidHistoryId: number): void {
    this.bidsService.getBidArchiveByHistoryId(bidHistoryId).subscribe(
      (result: any) => {
        if (
          result &&
          result.BidArchive &&
          Array.isArray(result.BidArchive) &&
          result.BidArchive.length > 0
        ) {
          this.bidArchiveRows = result.BidArchive;
          this.offers = [...this.bidArchiveRows];
          this.offer = this.offers[0];
  
          // ✅ Safely check if RampRate exists
          if (result.RampRate) {
            this.hasRampRate = true;
            this.rampRate = result.RampRate;
          } else {
            this.hasRampRate = false;
            this.rampRate = {
              BidRampRateArchiveID: null,
              BidHistoryID: bidHistoryId,
              RRMW1: null, RRMW2: null, RRMW3: null, RRMW4: null, RRMW5: null,
              RRRU1: null, RRRU2: null, RRRU3: null, RRRU4: null, RRRU5: null,
              RRRD1: null, RRRD2: null, RRRD3: null, RRRD4: null, RRRD5: null
            };
          }
  
          // ✅ Correct optional check
          if (!this.dateFormatted && this.offer && this.offer.Date) {
            this.dateFormatted = this.formatDate(this.offer.Date);
          }
          ...
          alert('Bid history is unavailable. Viewing function skipped due to missing data');
          window.close();
        }
      },
      error => {
        console.error('Failed to load BidArchive:', error);
        alert("Viewing disabled: bid history is missing or inaccessible");
        this.hasRampRate = false;
      }
    );
  }
  ...
  upload(printPDFmodal:NgbModal) {
    ...
    this.unitService.logBid(this.unitId, this.unitNumber, this.offers, this.controlModeValue).subscribe(data => {
        console.log(this.unitId, this.unitNumber, this.offers, this.controlModeValue)
    });
  }
